...
<BODY>
    ...
    <?php
		...
		{
			header('Location: inscription.php?champs=pseudo');
		}
		...
		{
			header('Location: inscription.php?champs=mdp');
		}
		elseif (!isset($_POST['sexe']) || empty($_POST['sexe'])) 
		{
			header('Location: inscription.php?champs=error');
		}
        elseif (isset($_POST['email']) &&  !empty($_POST['email']) && filter_var(strip_tags($_POST['email']), FILTER_VALIDATE_EMAIL))
        {
        	$_POST['profil'] = '1'; //c'est un utilisateur
        	$_POST['adresse'] = ''; //c'est un utilisateur
        	$_POST['type'] = ''; //c'est un utilisateur
			...
			if($donnees['ici'])
			{
				header('Location: inscription.php?champs=base');
			}
            $req = $bdd->prepare('INSERT INTO membres(pseudo, mdp, email, sexe, profil, adresse, type)
								 VALUES (:pseudo, :mdp, :email, :sexe, :profil, :adresse, :type)');
			//$hash = ;
            $req->execute(array(
